...
This private development repository additionally includes the original Meta checkpoint, configuration and MHR rig under their accompanying agreements. NuiSensorLib is vendored under MIT with provenance. Models are stored using Git LFS. Runtime DLLs and historical alternative model weights are not committed. Native runtime terms apply when those dependencies are downloaded and packaged.
