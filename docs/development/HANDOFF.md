...
## Private development — 2026-09-17: continuous translation between observations

User reported stop-go straight leg movement (lateral, vertical and fore/aft) with native SteamVR. Offline constant-speed body/estimator/output tests reproduced a presentation fault even with clean poses and depth: the internal joint-velocity feedback deadband reduced 3–6 cm/s movement to zero. Positions continued moving at camera cadence, leaving the final filter to repeatedly catch up. At 15/30 Hz, rendered speed standard deviation divided by commanded speed was about 0.37–0.78 for those ramps. This is a synthetic continuity metric, not measured live accuracy.

`PresentationVelocity` now estimates signed coherent velocity from the final corrected tracker positions, including tracker offsets and contact corrections. The estimate is used for camera-led fitted-body output, bypassing the internal joint-velocity deadband. A 70 ms signed trend/activity estimate suppresses alternating noise by directional coherence rather than suppressing slow motion solely because it is slow. Gaps, invalid tracks, source changes and player changes reset history; planted feet retain zero prediction velocity. Source history must be captured before clearing each frame's learned flags. Internal body fitting, contact decisions, calibration, angular output, final driver smoothing, the existing 100 ms maximum prediction horizon and 240 ms validity deadline remain unchanged. SteamVR and OSC share the correction through tracker state; zero SteamVR compositor velocity is retained.

Validation: constant-speed 3/6/20 cm/s ramps at 15/30 Hz now preserve expected prediction velocity and satisfy <2% rendered speed variation. Lifted-leg back-and-forth tests along all three axes at 6 cm/s have <0.006 m/s steady velocity error after reversal settling. Alternating-position noise, gap/reacquisition, existing floor/contact and OVR invariants are covered by the offline suites. Full Release build and all ten CTest suites pass. No camera recording or live output was enabled; user assessment is still required to establish how much of the reported uneven movement this resolves.

## Private development — 2026-09-17: saved alignment lifecycle

`SavedAlignment` separates the last accepted per-sensor calibration from the live calibration's validity and an in-progress attempt. Previously camera reconnect/capture-epoch changes and VR restarts cleared a shared accepted flag, so Confirm saved alignment could report no accepted calibration despite a good transform. Same-sensor reconnects now pause output but retain the accepted snapshot; cancelled or unsuccessful attempts do not replace it. A different sensor loads its own saved calibration. Camera motor changes and manual device-offset changes still invalidate the accepted snapshot.

Confirm requires a live, recent camera frame, a settled camera and a valid VR reference, but does not require the headset/controllers to be currently pose-tracked. Headset removal with an unchanged VR reference can reuse the saved transform. Same-session OVR drag preserves the original standing-to-raw reference; confirmation never rebinds it to the moved playspace. A real live epoch/origin change is rejected with an explicit realignment message. Start recreates the VR connection and forgets the transient reference, retaining the saved transform; explicit confirmation then requires the existing user assumption of a fixed camera and reset OVR offsets, as after app restart. No saved file format change or automatic output re-enabling was introduced.

Offline controller suite covers headset pose loss, reference loss/recovery, OVR movement, real epoch changes, failed-attempt isolation, invalidation, capture restart, per-sensor replacement, bad quality and save/load/confirm. Full Release build and all ten CTest suites pass. No live headset or camera test was performed; the user's exact removal/re-wear event sequence has not been measured.


## Private development — 2026-09-17: calibration retry and overlay recovery

The overlay inferred capture from a countdown threshold, which incorrectly labelled the three-second settling period as capture. View now carries the actual AlignmentCue phase. Pose instructions are shared between speech/desktop and the headset card: comfortable lower-chest/waist positions, elbows low, no wide stretch or overhead reach. Four offset-learning poses plus the independent check remain; fitting tolerances, sampling and tracking behavior are unchanged. Capture cards show each wrist's status and a countdown to the existing 20-second timeout rather than apparently waiting indefinitely.

Readiness now tracks release/press separately for each controller. One held trigger no longer blocks the other from retrying; stale/untracked inputs disarm that controller until a fresh release. Every new pose still requires readiness and the three-second settling period. Offline tests run twenty consecutive failures using one trigger while the other stays held, then verify successful recovery.
...
## Private development — 2026-09-17: compact front-of-body calibration poses

Refined the shared pose instructions to keep wrists ahead of the torso and hands below shoulder height: (1) just above waist, a handspan forward, diagonal down; (2) lower chest, a little farther forward, point forward; (3) slightly lower and just outside shoulder width, diagonal forward/out; (4) lower chest and shoulder width, a handspan forward, tilt up with elbows low; (5) independent lower forward reach, just above waist and farther out than pose 1. Exact angles/distances are not required; maintain the same normal grip. Shared titles now accompany shared instructions in the headset card. Diagram labels explicitly distinguish controller pointing direction from arm height. README's stale T-pose alignment instruction was corrected.

No solver tolerances, observation gates, ready/settle timing, saved calibration format, tracker output or overlay rendering lifecycle changed. The four fitting poses preserve pitch and yaw variation for unknown wrist offsets; the fifth remains held out from fitting. Synthetic guided tests now use these compact, equal-height left/right wrist positions at 15/30 Hz, with/without floor support and controller rotations at 70%/100% of nominal. They verify exact clean transform/offset recovery, rejection of a bad check, retry recovery and preservation of OVR coordinate transforms. Existing noisy/rebased-controller and occlusion tests use the compact positions as well. These establish solver compatibility, not measured live Kinect visibility or a claim of universally optimal poses.

Validation: targeted controller suite and full Release build plus all ten CTest suites before deployment. No camera recording or live tracker output enabled.


## Private development — 2026-09-17: natural grip and overlay submission ordering

User reported missing/stalled headset cards after the GPU-texture change. Corrected a concrete GPU submission defect: Flush now runs AFTER SetOverlayTexture, including on failed submissions, so compositor copy work queued by that call is submitted before the thread goes idle. The previous pre-submit-only Flush could leave that work pending. The shared presentOverlayFrame helper is used by production and tested for upload/submit/flush/show ordering, failure flushing, and no redundant ShowOverlay. This is a plausible explanation, not a reproduced live root-cause claim. Valve API reference: https://github.com/ValveSoftware/openvr/wiki/IVROverlay::SetOverlayTexture .

Overlay keys are per-process, preventing accidental reuse of another process's renderer-owned handle. A changed VR initialization token discards transient handles and GPU resources and forces a fresh card; overlay setup errors are checked. Diagnostics now include displayed stage, successful submission count, failure count and last OpenVR/HRESULT error (no image data). A successful submission is not proof of headset presentation. Runtime error reporting is retained for user follow-up if the issue persists.

Removed the downward pose, replacing it with a normal forward hold with straight wrists. Instructions for ALL poses now refer to natural forearm/elbow movement while keeping a normal controller grip: relaxed hold, easy reach, gently opening forearms, bending elbows as if lifting mugs, then a relaxed independent check. The diagram's first-pose down arrows were removed. Pose 4 no longer asks for isolated wrist tilting. Compact-pose synthetic tests now use a neutral first-pose orientation, retaining smaller-angle cases, 15/30 Hz, floor/no-floor, OVR transforms, noisy wrists, unfamiliar controller axes, retry recovery and independent holdout rejection. All ten CTest suites and full Release build are required before deployment. Actual ergonomic comfort and headset display stability remain live verification items. No camera recording or live tracker output was enabled.


## Private development — 2026-09-17: five distinct hand-position poses

Replaced ambiguous grip metaphors and multi-part rotation instructions with a sequence named for visible hand positions: waist, apart (shallow V), chest, forward reach, then left chest/right waist for independent validation. Maintain the same controller grip, moving at elbows/shoulders with comfortable wrists. This keeps yaw and pitch variation without asking users to aim a model-specific controller axis. The final check is deliberately asymmetric and remains excluded from fitting. A failed fit now repeats stages 2–4 (preserving stage 1), so insufficient outward rotation can actually be recaptured in the reordered sequence.

The headset diagram now shows a torso, arms and hand positions in the user's left/right convention instead of identical floating controller capsules. Forward reach has its own cue. Shared text, speech and README describe the same sequence. Compact guided tests were updated to the new order and final asymmetric wrist observations; clean/noisy, 15/30 Hz, smaller-angle, OVR and holdout/retry checks pass. This verifies numerical compatibility, not live ease of use. Rendering transport, tracking and fit acceptance thresholds were not changed. Full Release build and all ten CTest suites required before deployment; no live camera or tracker test run.
