...
Native SteamVR waist/feet output is the default. OSC remains optional. Preview 23 binds a live standing-to-raw physical reference at alignment/confirmation. Outgoing trackers use that fixed reference; incoming VR devices and interpolated poses are rebased into it. This prevents OVR standing-space movement being cancelled or entering body correction. Smooth velocity decay cushions tracker delivery across intermittent frame delay spikes without hard freeze jumps. Ordinary same-universe chaperone edits do not invalidate the reference. A SteamVR restart or changed hardware universe does; a running session then requires Align to VR again. The live reference is intentionally not serialized. After an app restart, reset OVR offsets before confirming saved calibration.
